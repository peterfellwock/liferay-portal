...
"lportal.isnull(?)";
    ...
    /**
     * Returns <code>true</code> if Hibernate is connecting to a DB2
database.
     *
     * @return <code>true</code> if Hibernate is connecting to a DB2
database
     */
    ...
    /**
     * Returns <code>true</code> if Hibernate is connecting to a MySQL
database.
     *
     * @return <code>true</code> if Hibernate is connecting to a MySQL
database
     */
     ...
     * http://thedailywtf.com/forums/thread/26879.aspx for more
information on
     * this nasty bug.
     *
     ...
     * database.
     *
     * @return <code>true</code> if Hibernate is connecting to a Sybase
database
     */
        ...
        try {
            if (Validator.isNotNull(functionIsNull) &&
                ...
                if (_log.isDebugEnabled()) {
                    _log.debug(
                        "functionIsNull is manually set to " +
functionIsNull);
                    _log.debug(
                ...
                if (dbName.startsWith("DB2")) {
                    ...
                    if (_log.isInfoEnabled()) {
                        _log.info("Detected DB2 with database name " +
dbName);
                    }
                }
                    ...
                    if (_log.isInfoEnabled()) {
                        _log.info("Detected HSQL with database name " +
dbName);
                    }
                }
                    ...
                    if (_log.isInfoEnabled()) {
                        _log.info(
                            "Detected Informix with database name " +
dbName);
                    }
                }
...
dbName.equals("ASE")) {
                    ...
                    if (_log.isInfoEnabled()) {
                        _log.info(
                            "Detected PostgreSQL with database name " +
dbName);
                    }
                }
                else {
                    if (_log.isDebugEnabled()) {
                        _log.debug(
                            "Unable to detect database with name " +
dbName);
                    }
                }
            }
        ...
        if (_sqlPool == null) {
            ...
            System.out.println(">CUSTOMSQL sqlPool.size::" +
_sqlPool.size());
            System.out.println(">XXXXX CUSTOMSQL: _sqlPool.clear()");
            System.out.println(">XXXXX CUSTOMSQL: _sqlPool.clear()");
            System.out.println(">XXXXX CUSTOMSQL: _sqlPool.clear()");
            System.out.println(">XXXXX CUSTOMSQL: _sqlPool.clear()");
            System.out.println(">XXXXX CUSTOMSQL: _sqlPool.clear()");
        ...
        try {
            ...
            System.out.println(">CUSTOMSQL: classloader::" +
classLoader.toString());
            System.out.println(">CUSTOMSQL: classloader::PortalClassLoaderUtil.getClassLoader():");
           
System.out.println(PortalClassLoaderUtil.getClassLoader().toString());
            ...
            System.out.println("----_sqlPool.size::" + _sqlPool.size()+
"--");
    ...
    public String replaceAndOperator(String sql, boolean andOperator) {
        ...
        sql = StringUtil.replace(
            sql, new String[] {"[$AND_OR_CONNECTOR$]",
"[$AND_OR_NULL_CHECK$]"},
    ...
    public String replaceGroupBy(String sql, String groupBy) {
        ...
        if (x != -1) {
            ...
            if (y == -1) {
                sql = sql.substring(0, x +
_GROUP_BY_CLAUSE.length()).concat(
                    groupBy);
            }
            else {
                sql = sql.substring(0, x +
_GROUP_BY_CLAUSE.length()).concat(
    ...
    public String replaceKeywords(
        ...
        if (ArrayUtil.isEmpty(values)) {
            return StringUtil.replace(sql, oldSql.toString(),
StringPool.BLANK);
        }
...
newSql.toString());
    ...
    protected String[] getConfigs() {
        ...
        System.out.println(">CUSTOMSQL: Is portal and util-javaclassloaders same:" + (PortalClassLoaderUtil.getClassLoader() == classLoader));
        ...
        if (classLoader.getResource("portlet.properties") != null) {
            System.out.println(">CUSTOMSQL: portlet.properties availableLOAD");
...
"META-INF/custom-sql/default.xml"
        ...
        System.out.println(">CUSTOMSQL: DEFAULT TO BASE");
        return new String[] {
            "custom-sql/default.xml", "META-INF/custom-sql/default.xml"
        };
    }

    protected String insertWildcard(String keyword, WildcardMode
wildcardMode) {
    ...
    protected void read(ClassLoader classLoader, String source)
        ...
        try (InputStream is = classLoader.getResourceAsStream(source)) {
            if (is == null) {
                System.out.println(">CUSTOMSQL: Unable to read:" + source);
                return;
            }
            ...
            for (Element sqlElement : rootElement.elements("sql")) {
                ...
                if (Validator.isNotNull(file)) {
                    System.out.println(">CUSTOMSQL:" + source + " saysload::" + file);
