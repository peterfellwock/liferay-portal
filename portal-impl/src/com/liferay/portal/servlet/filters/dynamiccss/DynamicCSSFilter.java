...
public class DynamicCSSFilter extends IgnoreModuleRequestFilter {
	...
	protected Object getDynamicContent(
		...
		if (resourceURL == null) {
			String strip = PortalWebResourcesUtil.getContextPath(PortalWebResourceConstants.RESOURCE_TYPE_CSS);
			String modifiedRequestPathString = requestPath;

			if(requestPath.startsWith(strip)){
				modifiedRequestPathString = requestPath.substring(strip.length(), requestPath.length());
			}

			resourceURL = PortalWebResourcesUtil.getServletContextResource(modifiedRequestPathString);
