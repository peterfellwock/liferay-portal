...
>
	...
	if (portletName.equals(PortletKeys.MY_SITES_DIRECTORY)) {
		...
		userGroupParams.put("userGroupsGroups", SitesUtil.filterGroups(groups, PropsUtil.getArray(PropsKeys.MY_SITES_DIRECTORY_SITE_EXCLUDES)));
	}
	...
	<liferay-ui:search-container-results>
		<c:choose>
			<c:when test="<%= portletName.equals(PortletKeys.DIRECTORY) && GetterUtil.getBoolean(PropsUtil.get(PropsKeys.USER_GROUPS_INDEXER_ENABLED)) && GetterUtil.getBoolean(PropsUtil.get(PropsKeys.USER_GROUPS_SEARCH_WITH_INDEX)) %>">
				<%@ include file="/user_groups_admin/user_group_search_results_index.jspf" %>
			</c:when>
			<c:otherwise>
				<%@ include file="/user_groups_admin/user_group_search_results_database.jspf" %>
			</c:otherwise>
		</c:choose>
	</liferay-ui:search-container-results>
